# game-list

A collection of game prototypes.

---

## 🔫 Turn Tanks

**Description:** Turn-based tank battle game. Test of basic gameplay loop. Not true multiplayer or mobile ready yet.

- 🚀 [https://turn-tanks.vercel.app/](https://turn-tanks.vercel.app/)
- 💻 [https://github.com/sfaber34/turn-tanks](https://github.com/sfaber34/turn-tanks)

---

## 🛩️ Lift

**Description:** A threejs glider sim. Basic controls and flight physics in place (physics need more work). Terrain needs work. Controls work best on mobile (desktop binary input sucks).

- 🚀 [https://lift-self.vercel.app/](https://lift-self.vercel.app/)
- 💻 [https://github.com/sfaber34/lift](https://github.com/sfaber34/lift)

---

## 🚘 Smash Fest

**Description:** Top down demolition derby game. Has basic multiplayer (partykit, no true lobbies). No win conditions yet, just a driving/party kit test. Note: there's a bug that makes desktop users drive much faster that mobile users (mobile speed is better).

- 🚀 [https://smash-fest-eight.vercel.app/](https://smash-fest-eight.vercel.app/)
- 💻 [https://github.com/sfaber34/smash-fest](https://github.com/sfaber34/smash-fest)

---

## 🎲 Roll House

**Description:** A dice rolling game with house economics baked in.

- 🚀 [https://rollhouse.austingriffith.eth.link/](https://rollhouse.austingriffith.eth.link/)
- 💻 [https://github.com/austintgriffith/house-pool](https://github.com/austintgriffith/house-pool)

---

## 🚗🛻 Demolition

**Description:** Top down demolition derby game with AI drivers and car stats.

- 🚀 [https://demolition-zakgriffith-zakgriffiths-projects.vercel.app](https://demolition-zakgriffith-zakgriffiths-projects.vercel.app)
- 💻 [https://github.com/ZakGriffith/demolition](https://github.com/ZakGriffith/demolition)

---

## ☄️ Max Extract

**Description:** Smart contract orchestration game. Players write contracts to enable mining in their sectors and earn credits.

- 🚀 [https://extract.fi/](https://extract.fi/)
- 💻 [https://github.com/austintgriffith/max-extract?tab=readme-ov-file](https://github.com/austintgriffith/max-extract?tab=readme-ov-file)

---

## Another Game Title

**Description:** Brief description of another game goes here.

- 🚀 [Live Demo](https://another-game.vercel.app)
- 💻 [Repository](https://github.com/username/another-repo)

---
